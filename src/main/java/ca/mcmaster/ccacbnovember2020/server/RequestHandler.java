...
public class RequestHandler implements Runnable{
        ...
        synchronized ( Server.responseMap) {
            if (Server.responseMap.isEmpty()){
                ...
                boolean areAllworkersHaveCompletedTheirAssignment =  
                        (numIdleClients==Parameters.NUM_WORKERS && Server.leafPool_FromRampUp.size()==ZERO);
                        
                boolean doLoadBalance = true;
                if (numIdleClients<Parameters.NUM_WORKERS && numIdleClients> ZERO && Server.globalIncombent<= Server.lowestBoundOfAllClients ){
                ...
                boolean isJustAfterRampup = numIdleClients==Parameters.NUM_WORKERS && Server.leafPool_FromRampUp!=null &&
                        Server.leafPool_FromRampUp.size() == Parameters.NUM_WORKERS;
    ...
    private boolean waitForAllClients () throws InterruptedException{
        ...
        for  (int limit = ZERO; limit < SIXTY * TWO*TWO * SIXTY; limit ++ ) {
            ...
            if ( countOfRecieved <  Parameters.NUM_WORKERS){
                //sleep for a second
